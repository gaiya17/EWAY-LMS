...
type ValidatableField = Exclude<keyof FormData, 'agreeToTerms'>;
...
export function RegistrationPage({
  ...
  useEffect(() => {
    ...
    (Object.keys(formData) as Array<keyof FormData>).forEach((key) => {
      if (key !== 'agreeToTerms') {
        const fieldKey = key as ValidatableField;
        const value = formData[fieldKey] as string;
        if (touched[fieldKey] || value) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We do not call onRegisterSuccess() here because the user must verify their email first
    } catch (error) {
            ...
            {submitSuccess ? (
              <div className="text-center py-8">
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-400 text-white font-semibold hover:shadow-[0_0_20px_rgba(99,102,241,0.4)] transition-all"
                >
                  Return to Login
                </button>
              </div>
              ...
              {submitSuccess && <p className="text-sm text-green-400 text-center">{submitSuccess}</p>}
