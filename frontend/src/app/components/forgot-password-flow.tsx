...
import { Mail, Lock, Eye, EyeOff, CheckCircle, ArrowLeft, Shield } from 'lucide-react';
import { resetPassword, sendResetCode, verifyResetCode } from '../lib/auth';
...
export function ForgotPasswordFlow({ onBackToLogin }: ForgotPasswordFlowProps) {
  ...
  const handleSendResetLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage('A reset code was sent if the account exists.');
      setResendTimer(60);
      setCurrentStep('verify');
    } catch (error) {
      setErrors({
        ...errors,
        email: error instanceof Error ? error.message : 'Unable to send reset code',
      });
    } finally {
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage('Reset code verified. Create your new password.');
      setCurrentStep('reset');
    } catch (error) {
      setErrors({
        ...errors,
        code: error instanceof Error ? error.message : 'Invalid reset code',
      });
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleResendCode = async () => {
    ...
    try {
      setIsSubmitting(true);
      await sendResetCode({ email });
      setStatusMessage('A new reset code was sent if the account exists.');
      setResetCode(['', '', '', '', '', '']);
      setResendTimer(60);
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (!newErrors.password && !newErrors.confirmPassword) {
      try {
        ...
        setErrors({
          ...newErrors,
          password: error instanceof Error ? error.message : 'Password reset failed',
        });
      } finally {
        setIsSubmitting(false);
      }
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <GlassCard className="p-8 md:p-10">
            ...
            {statusMessage && <p className="mt-4 text-center text-sm text-cyan-300">{statusMessage}</p>}
          </GlassCard>
        </div>
      </div>
    </div>
